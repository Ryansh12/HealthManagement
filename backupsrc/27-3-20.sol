...
contract Doctor {
    ...
    string memory postalcodee, string memory countryy, string memory passwordd) public returns (bool) {
        if(check[msg.sender]) {
            AddressMapping am = AddressMapping(0x7De9CeCDAB2125224028Ef456B4BCF2aDeBAb5DD);
...
contract MedicalRecord {
    ...
    function isDoctorAllowed(address add) public view returns(bool) {
        Mapping mp = Mapping(0x7FF5f5607b5f609d3fa63F6Cc0c16d90ab68eA6c);
...
contract AddressMapping {
    ...
    function createOwner(string memory pass) public returns(address) {
...
contract Owner {
    ...
    function addNewInsurance(address add) public returns(bool) {
    ...
    function createInsurance(string memory name, int aadhar, int amount, string memory insDetailss, string memory documentss, int setInsuranceAddresss) public returns(address)  {
        if( msg.sender == manager ) {
    ...
    function pendingRequest(address ownerAddd, address selfAddd, string memory titlee, string memory detailss, string memory fileHash, bool doctorr, bool ownerrrr, uint256 index) public returns(bool) {
    ...
    function approveCancelRequest(uint256 index, bool flag) public returns(bool) {
        if(msg.sender == manager) {
            if(flag == true) {
...
contract Insurance {
    ...
    function applyClaim(string memory fileHash, string memory titlee, string memory detailss) public returns(bool) {
        Request memory newRequest = Request( {
            ownerAdd: ownerr,
